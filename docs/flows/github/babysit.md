...
>
> Control returns to the user at exactly three points: **(a)** a terminal outcome — the PR merged, or was closed without merging; **(b)** a **pause & hand back** — an out-of-scope review ask or a non-clean merge the loop cannot resolve (*Stuck = pause & hand back*), or the **20-tick cap** (*Tick cap*); **(c)** an explicit user halt. **Everywhere else — including every ordinary idle tick — the tick MUST end with an automatic next wake armed**: a standing schedule or a firing self-wake (*Scheduling* → *Self-continuation*). `/babysit` is always automatic — there is no on-demand / manual "re-invoke to continue" mode; if the host can arm neither automatic trigger, report the incompatibility (do not run a one-tick manual loop). Ending a tick with "waiting for review" and no automatic wake armed is the stall this contract exists to prevent.
...
`/babysit` is **always automatic** — every non-terminal tick fires the next one on a timer, with **no** user action in between. There is **no on-demand / "re-invoke to continue" mode**: the loop never runs one-tick-per-invocation and never asks the user to poke it forward. A **stall** is a tick that ends without an automatic next wake armed. Two automatic arrangements are valid — use the first the host supports:

1. **Standing schedule (preferred; unattended).** A durable cron (`CronCreate durable: true`), an OS timer, or Routines fires each tick automatically; the agent does its tick and yields. Resilient to a usage-limit kill (`core/loop` → *Usage-Limit Resilience*). Use this whenever the host offers it.
2. **Self-arm in-session** — `ScheduleWakeup` with the interval + a prompt that re-enters the loop on the same PR — where the host actually delivers the self-wake automatically.

**If the host can arm neither an automatic standing schedule nor a firing self-wake, do not start a degraded manual loop — report the incompatibility at setup** — the same honest report-at-setup discipline `core/loop` → *Durability* mode 3 applies to an unpersistable scratchpad, here applied to the no-fireable-scheduler axis: state that the watch cannot run automatically on this host and ask the user to pick a scheduler that fires (a durable cron, or a platform whose self-wake works). Reporting an incompatibility is honest; silently running one tick and waiting for a nudge is not — that is the stall this contract forbids.

Confirm an automatic trigger is actually armed before the first status line: "I'll keep checking" with no standing schedule and no firing self-wake **is** the stall. Stop arming the next wake **only** at the hand-back points in *The loop must never stall*.
...
This cap **replaces** `core/loop`'s wall-clock skip-streak nag for this loop: a fixed tick count is the concrete, interval-independent bound `/babysit` uses instead of "drift after N idle ticks." Reaching the cap is a hand-back, not an on-demand mode — the automatic loop ran its 20 ticks and pauses; a fresh `/babysit <pr>` starts a new automatic run of 20.
...
- **`/babysit` is always automatic — there is no on-demand / manual-re-invoke mode.** Every non-terminal tick arms an automatic next wake (standing schedule or firing self-wake). If the host can arm neither, **report the incompatibility at setup** (the same report-at-setup discipline as `core/loop` → *Durability* mode 3, applied to the scheduler axis) — never fall back to a one-tick-per-invocation manual loop.
- **An idle tick re-arms the automatic wake; it never silently stops the loop.** "Waiting for review" is the normal state — end every such tick with the next wake armed (*Scheduling* → *Self-continuation*). Stopping without a terminal / hand-back / user-halt reason is this skill's #1 failure mode. A status line with no action and no armed next wake is the tell.
